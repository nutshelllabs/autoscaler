...
const app = rewire('../../scaling-methods/cautious.js');
...
/**
 * Helper to stub base module for testing
 * @param {Object} spanner - Spanner object
 * @param {Object} metric - Metric object
 * @param {boolean} metricValueWithinRange - Whether metric is within range
 * @return {Object} Callback stub
 */
...
describe('#cautious.calculateSize', () => {
  ...
  it('should return the number of processing units increased by overloadStepSize if the instance is overloaded', () => {
    ...
    const callbackStub = stubBaseModule(
      spanner,
      {value: OVERLOAD_THRESHOLD + 1, threshold: 65, name: OVERLOAD_METRIC},
      false,
    );
  ...
  it('should return PUs decreased by 1000 when scaling-in, and the size is >1000 PUs, and stepSize is <1000 PUs', () => {
    const spanner = createSpannerParameters(
      {currentSize: 11000, stepSize: 100},
      true,
    );
    const callbackStub = stubBaseModule(
      spanner,
      {value: 5, threshold: 65},
      false,
    );
  ...
  it('should respect minSize', () => {
    const spanner = createSpannerParameters(
      {currentSize: 8000, stepSize: 6000, minSize: 7000},
      true,
    );
    const callbackStub = stubBaseModule(
      spanner,
